...
| 113 | **Upgrade bootstrap one-liner** | Created `upgrade_bootstrap.sh` that clones/updates the repo to a temp directory and runs `upgrade.sh` from there. Users can upgrade with a single command: `curl -sSL https://raw.githubusercontent.com/HansvanMeer/pyMC_WM1303/main/upgrade_bootstrap.sh \| sudo bash`. Updated README.md with one-liner instructions in the Upgrading section. |
