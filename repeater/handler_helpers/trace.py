...
class TraceHelper:
    ...
    async def process_trace_packet(self, packet) -> None:
        ...
        try:
            ...
            if should_forward:
                ...
                _is_dup = bool(
                    self.repeater_handler and self.repeater_handler.is_duplicate(packet)
                )
                self._log_no_forward_reason(packet, trace_bytes, hash_width, is_dup=_is_dup)
                if (
                    self.on_trace_complete
                    and self._is_trace_complete(packet, trace_bytes, hash_width)
                    and self.repeater_handler
                    # NOTE: do not gate on _is_dup here. inject_packet() calls
                    # repeater_handler(local_transmission=True) before enqueuing to the
                    # router, which registers the packet in the dedup cache before
                    # TraceHelper ever runs. The dedup flag is therefore always True for
                    # bridge-received TRACE_RESPs and cannot be used as a delivery guard.
                    # Duplicate delivery of the 0x89 frame is benign (app handles it).
                ):
                    try:
    ...
    def _log_no_forward_reason(self, packet, trace_bytes: bytes, hash_width: int, is_dup: bool = False) -> None:
