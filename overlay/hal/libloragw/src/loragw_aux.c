...
uint32_t lora_packet_time_on_air(const uint8_t bw, const uint8_t sf, const uint8_t cr, const uint16_t n_symbol_preamble,
    ...
    /* Get bandwidth divider relative to 62.5KHz base
     * Formula: t_symbol_us = (1 << sf) * 16 / bw_pow
     *   BW_62K5HZ: bw_pow=1  -> (2^SF) * 16 us  (= 2^SF / 62500 * 1e6)
     *   BW_125KHZ: bw_pow=2  -> (2^SF) * 8  us  (= 2^SF / 125000 * 1e6)
     *   BW_250KHZ: bw_pow=4  -> (2^SF) * 4  us  (= 2^SF / 250000 * 1e6)
     *   BW_500KHZ: bw_pow=8  -> (2^SF) * 2  us  (= 2^SF / 500000 * 1e6)
     */
    switch (bw) {
        case BW_62K5HZ:
            bw_pow = 1;
    ...
    t_symbol_us = (1 << sf) * 16 / bw_pow; /* 2^SF / BW , in microseconds */

    /* Packet parameters */
    H = (no_header == false) ? 1 : 0; /* header is always enabled, except for beacons */
    /* Low datarate optimization: enable when symbol duration > 16ms
     * This is BW-dependent: at BW62.5 LDRO is needed from SF10+,
     * at BW125 from SF11+, at BW250 only SF12 */
    DE = (t_symbol_us > 16000) ? 1 : 0;
