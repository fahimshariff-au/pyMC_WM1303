...
class RepeaterDaemon:
    ...
    async def _load_companion_identities(self) -> None:
        ...
        for comp_config in companions_config:
            try:
                ...
                # Wire to BridgeEngine for raw RX push (0x88) if already initialised.
                # In WM1303 mode the Dispatcher's add_raw_rx_subscriber (registered above)
                # never fires because raw RF packets flow through BridgeEngine._rx_loop,
                # not the pymc_core Dispatcher receive path.  Direct wiring here covers
                # the case where bridge_engine is ready before companions are loaded.
                # The reverse case (bridge_engine starts after) is handled by
                # _wire_companion_frame_servers_to_bridge(), called from run().
                if self.bridge_engine is not None:
                    try:
                        self.bridge_engine.add_companion_frame_server(frame_server)
                    except Exception as _be_e:
                        logger.debug("Failed to wire frame server to BridgeEngine: %s", _be_e)
    ...
    async def add_companion_from_config(self, comp_config: dict) -> None:
        ...
        await frame_server.start()
        self.companion_frame_servers.append(frame_server)
        # Wire to BridgeEngine for raw RX push (0x88) — hot-reload path
        if self.bridge_engine is not None:
            try:
                self.bridge_engine.add_companion_frame_server(frame_server)
            except Exception as _be_e:
                logger.debug("Hot-reload: failed to wire frame server to BridgeEngine: %s", _be_e)
    ...
    def _wire_companion_frame_servers_to_bridge(self) -> None:
        """Wire all companion frame servers to the BridgeEngine for raw RX push (0x88).

        Call this once after self.bridge_engine is set (typically in run() immediately
        after starting the bridge engine).  Safe to call multiple times — BridgeEngine
        appends to a list so duplicate calls would double-push; guard with a flag or
        only call once per startup.  Covers the common case where companion identities
        are loaded during initialize() before the BridgeEngine is started.
        """
        if self.bridge_engine is None:
            return
        if not self.companion_frame_servers:
            return
        for fs in self.companion_frame_servers:
            try:
                self.bridge_engine.add_companion_frame_server(fs)
            except Exception as _e:
                logger.debug("Failed to wire frame server to BridgeEngine: %s", _e)
        logger.info(
            "BridgeEngine: %d companion frame server(s) wired for raw RX push (0x88)",
            len(self.companion_frame_servers),
        )
    ...
    def _init_wm1303_bridge(self):
        ...
        # Wire companion frame servers for raw RX push (0x88).
        # Companion identities are loaded in initialize() before the BridgeEngine
        # is created here, so frame servers exist but are not yet wired.
        self._wire_companion_frame_servers_to_bridge()
