...
class TraceCollector:
    ...
    def __init__(self, maxlen: int = MAX_TRACES):
        ...
        # OrderedDict keyed by pkt_hash for fast lookup + insertion order
    ...
    def trace_event(self, pkt_hash: str, step_name: str,
        ...
        If a trace for this hash already exists (within TTL), the step
        is appended.  Otherwise a new trace entry is created.
        """
        ...
        with self._lock:
            trace = self._traces.get(pkt_hash)
            ...
            if trace is None:
                ...
                self._traces[pkt_hash] = trace
            else:
                # Move to end (most recent)
                self._traces.move_to_end(pkt_hash)
...
def get_collector() -> TraceCollector:
    ...
    if _collector is None:
        ...
        logger.info('PacketTrace: collector initialized (maxlen=%d)', MAX_TRACES)
    return _collector
