...
class Contact:
    ...
    # Per-node signal quality from last received advert (WM1303 overlay).
    last_rssi: Optional[int] = None    # dBm, None = unknown
    last_snr: Optional[float] = None   # dB, None = unknown
    ...
    def from_dict(
        ...
        *,
        ...
        if isinstance(lap, str) and lap:
            ...
            last_advert_packet = None
        # Extract per-node signal quality (WM1303 overlay).
        # Keys 'rssi'/'snr' come from NODE_DISCOVERED event_data;
        # 'last_rssi'/'last_snr' come from serialized Contact dicts.
        _raw_rssi = d.get("rssi", d.get("last_rssi"))
        last_rssi = int(_raw_rssi) if _raw_rssi is not None else None
        _raw_snr = d.get("snr", d.get("last_snr"))
        last_snr = float(_raw_snr) if _raw_snr is not None else None
        return cls(
            public_key=pub,
            name=name,
            adv_type=adv_type,
